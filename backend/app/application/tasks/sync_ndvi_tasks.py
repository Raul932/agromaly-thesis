...
@shared_task(
    bind=True,
    name="app.application.tasks.sync_ndvi_tasks.sync_ndvi_for_all_parcels",
    max_retries=0,
    acks_late=True,
)
def sync_ndvi_for_all_parcels(self) -> dict:
    """Dispatch individual NDVI sync tasks for every active parcel."""
    parcel_ids = asyncio.run(_async_list_active_parcel_ids())
    for pid in parcel_ids:
        sync_ndvi_for_parcel.delay(pid)
    logger.info("sync_ndvi_for_all_parcels dispatched %d tasks", len(parcel_ids))
    return {"dispatched": len(parcel_ids)}


async def _async_list_active_parcel_ids() -> list[str]:
